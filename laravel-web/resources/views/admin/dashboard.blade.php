...
<aside class="fixed left-0 top-0 hidden h-screen w-64 flex-col border-r border-slate-100 bg-slate-50 md:flex">
    ...
    <nav class="flex flex-1 flex-col gap-2 p-4">
        <a
            ...
            class="flex items-center gap-3 rounded-md border-t-2 border-yellow-500 bg-white px-4 py-3 font-semibold text-blue-700 shadow-sm transition-transform duration-200"
        >
            <span class="material-symbols-outlined">dashboard_customize</span>
            <span class="text-sm">Admin Console</span>
        </a>

        <span class="flex cursor-not-allowed items-center gap-3 px-4 py-3 text-slate-400">
...
<main class="min-h-screen md:ml-64">
    <header class="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-100 bg-white/80 px-6 backdrop-blur-md md:px-8">
        <div>
            ...
            <p class="text-[10px] font-medium uppercase tracking-[0.2em] text-slate-400">Academic Affairs Division</p>
        </div>
    ...
    <div class="mx-auto w-full max-w-screen-2xl p-6 md:p-8">
        <section class="mb-8 flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
            <div>
                <h1 class="mb-1 text-3xl font-extrabold tracking-tight text-on-surface">Scholarship Dashboard</h1>
                <p class="font-medium text-on-surface-variant">
                    Monitoring pengajuan mahasiswa, hasil machine learning, dan keputusan admin dalam satu halaman kerja.
                </p>
        ...
        <section class="mb-10 grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
            <div class="rounded-xl border border-slate-100 bg-white p-6 shadow-lg">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-black uppercase tracking-[0.2em] text-slate-400">Workflow Guardrail</p>
                ...
                <div class="mt-4 grid gap-3 md:grid-cols-3">
                    <div class="rounded-2xl bg-surface-container p-4">
                        <p class="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Input Mahasiswa</p>
                        <p class="mt-2 text-sm font-semibold text-on-surface">13 atribut ter-encoding + 1 PDF bukti dikirim lewat portal.</p>
                    </div>
                    <div class="rounded-2xl bg-surface-container p-4">
                        <p class="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Machine Learning</p>
                        <p class="mt-2 text-sm font-semibold text-on-surface">CatBoost menjadi rekomendasi utama, Naive Bayes menjadi pembanding.</p>
                    </div>
                    <div class="rounded-2xl bg-surface-container p-4">
                        <p class="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Final Label</p>
                        <p class="mt-2 text-sm font-semibold text-on-surface">Status `Verified` atau `Rejected` akan mengalir ke data training retrain.</p>
                    </div>
                </div>
            </div>

            <div class="rounded-xl border border-slate-100 bg-white p-6 shadow-lg">
                <p class="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">System Snapshot</p>
                <div class="mt-4 space-y-4">
                    <div class="rounded-2xl bg-surface-container p-4">
                        <p class="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Schema Source</p>
                    ...
                    <div class="rounded-2xl bg-surface-container p-4">
                        <p class="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Queue Priority Tinggi</p>
                        <p class="mt-2 text-2xl font-black text-error">{{ $applicationStats['high_priority_pending'] }}</p>
                    </div>

                    <div class="rounded-2xl bg-surface-container p-4">
                        <p class="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Training Correction</p>
                ...
                <table class="w-full border-collapse text-left">
                    <thead>
                        <tr class="bg-slate-50/50">
                            <th class="border-b border-slate-100 px-6 py-4 text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Applicant</th>
                            <th class="border-b border-slate-100 px-6 py-4 text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Submitted</th>
                            <th class="border-b border-slate-100 px-6 py-4 text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Status</th>
                            <th class="border-b border-slate-100 px-6 py-4 text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Priority</th>
                            <th class="border-b border-slate-100 px-6 py-4 text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Recommendation</th>
                            <th class="border-b border-slate-100 px-6 py-4 text-[10px] font-black uppercase tracking-[0.18em] text-slate-400 text-right">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-50">
                        @forelse ($applications as $application)
                            @php
                                ...
                                $studentName = $student?->name ?? 'Mahasiswa tanpa nama';
                            ...
                            <tr class="transition-colors hover:bg-slate-50/50">
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-[11px] font-medium text-slate-400">{{ $student?->email ?? 'Email belum tersedia' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5">
                                    <p class="text-sm font-medium text-on-surface">{{ $application->created_at?->format('d M Y, H:i') ?? '-' }}</p>
                                    <p class="text-[11px] text-slate-400">Schema v{{ $application->schema_version }}</p>
                                </td>
                                <td class="px-6 py-5">
                                ...
                                <td class="px-6 py-5">
                                    ...
                                    <p class="text-[11px] text-slate-400">
                                        ...
                                        @else
                                            Snapshot model belum siap
                                        @endif
                                    </p>
                                    @if ($snapshot?->disagreement_flag)
                                        <p class="mt-1 text-[11px] font-semibold text-error">CatBoost dan Naive Bayes memberi hasil berbeda.</p>
                                    @endif
                                </td>
                                <td class="px-6 py-5">
    ...
    <footer class="mt-auto w-full border-t border-slate-100 bg-white py-8">
        <div class="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-8 md:flex-row">
            <div class="text-left">
                <p class="mb-1 font-bold text-slate-900">KIP-K Management System</p>
